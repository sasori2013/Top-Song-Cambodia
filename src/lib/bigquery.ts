...
export async function getRankingDataFromBQ(): Promise<RankingResponse | null> {
  ...
  try {
    // Phase 1: Get critical dates (serial — everything depends on these)
    ...
    const [
      ...
      bq.query(`
        ...
        daily_increase AS (
          ...
          FROM daily_stats WHERE prev_views IS NOT NULL GROUP BY date
        ),
        growth_stats AS (
          SELECT
          ...
          FROM daily_increase WHERE date <= DATE '${latestDate}'
        ),
        weekly_agg AS (
          SELECT
          ...
          FROM daily_increase
          WHERE date <= DATE '${latestDate}' AND date > DATE_SUB(DATE '${latestDate}', INTERVAL 70 DAY)
          GROUP BY week_index
        )
        SELECT wa.weekly_volume, wa.week_index, gs.this_week, gs.last_week
      ...
      bq.query(`
        ...
        GROUP BY sm.genre ORDER BY daily_views DESC
      `),
      bq.query(`
        ...
        LEFT JOIN \`${DATASET_ID}.snapshots\` prev
          ON curr.videoId = prev.videoId AND prev.date = DATE_SUB(curr.date, INTERVAL 1 DAY)
        WHERE curr.date = DATE '${latestDate}'
        ORDER BY daily_views DESC LIMIT 5
      `),
      bq.query(`
        WITH weekly AS (
          SELECT
      ...
      bq.query(`
        ...
        WHERE genre IS NOT NULL AND genre != ''
          AND DATE(publishedAt) BETWEEN DATE_SUB(DATE '${latestDate}', INTERVAL 12 MONTH) AND DATE '${latestDate}'
        GROUP BY month, genre ORDER BY month ASC
      `),
      bq.query(`
        ...
        FROM \`${DATASET_ID}.songs_master\` sm
        JOIN (
          SELECT videoId, MAX(views) AS views FROM \`${DATASET_ID}.snapshots\` GROUP BY videoId
        ...
        GROUP BY month, sm.genre ORDER BY month ASC
      `),
      bq.query(`
        ...
        WHERE curr.date >= DATE_SUB((SELECT d FROM maxdate), INTERVAL 6 DAY)
          AND sm.genre IS NOT NULL AND sm.genre != ''
        GROUP BY sm.genre ORDER BY week_views DESC
      `),
      bq.query(`
        WITH daily_stats AS (
          SELECT date, videoId, views,
            LAG(views) OVER(PARTITION BY videoId ORDER BY date) as prev_views
          FROM \`${DATASET_ID}.snapshots\`
          WHERE date >= DATE_SUB(DATE '${latestDate}', INTERVAL 16 DAY)
        ),
        daily_increase AS (
          SELECT date,
            SUM(CASE WHEN views > prev_views THEN views - prev_views ELSE 0 END) as total_dv
        ...
        SELECT FORMAT_DATE('%m-%d', date) as date_label, total_dv
        FROM daily_increase
        WHERE date <= DATE '${latestDate}' AND date > DATE_SUB(DATE '${latestDate}', INTERVAL 14 DAY)
        ORDER BY date ASC
      `),
      getRegionalDataFromSheet(),
    ]);

    // Phase 3: Queries that depend on Phase 2 results (parallel where possible)
    const videoIds = rankingRows.map(r => r.videoId);
    const prevPrevSnapDate = prevPrevSnapRows[0]?.d || null;

    const [historyResult, prevActionResult] = await Promise.all([
      videoIds.length > 0
        ? bq.query(`
    ...
    const [prevActionRows] = prevActionResult;
    ...
    // Regional data: fallback to BQ if sheet was empty
    let regionalData = regionalDataRaw;
    ...
    const prevDailyActions = prevPrevSnapDate && prevActionRows[0] ? {
    ...
    const dailyTraffic = trafficRows.map((r: any) => ({
      date: r.date_label,
      value: Number(r.total_dv || 0),
    }));

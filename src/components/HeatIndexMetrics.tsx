        ...
        <span className="tabular-nums flex items-baseline justify-center">
            ...
            {suffix && (
                <span className="text-white/30 ml-2 relative inline-flex items-start">
                    ...
                    {suffix.includes('*') && (
                        <span className="text-[11px] md:text-[13px] text-white/40 leading-none select-none absolute -right-3 -top-1 md:-right-4 md:top-1 font-bold">*</span>
                    )}
                </span>
            )}
        </span>
                ...
                <div className="flex flex-col items-stretch gap-8 mb-6 w-full">
                    <div className="flex flex-col items-center text-center text-white">
                        <div className="text-5xl md:text-6xl font-extralight tracking-tighter leading-none tabular-nums flex items-center justify-center">
                            {growth > 0 && <span className="text-2xl md:text-3xl text-white/30 mr-1">+</span>}
                            <RollingValue value={growth} suffix="%*" />
                        </div>
                        <p className="text-[8px] md:text-[9px] font-bold text-white/30 uppercase tracking-[0.2em] mt-4">
                            WEEKLY COMPARISON <span className="text-white/20 ml-1 font-mono">VS LAST WEEK</span>
                        </p>
                    </div>
                    ...
                    <div className="w-full pt-4 border-t border-white/5">
                        <div className="flex flex-col h-full">
                                ...
                                {displayTrend.map((val, i) => (
                                    <div key={i} className="flex-1 flex flex-col justify-end items-center h-full group/bar relative">
                                        <div className="absolute -top-7 left-1/2 -translate-x-1/2 opacity-60 group-hover/bar:opacity-100 transition-opacity duration-300 pointer-events-none z-20">
                                            <div className="text-[8px] font-mono text-white/70 font-black whitespace-nowrap">
                            ...
                            <div className="flex justify-between px-2 mt-3">
                                {displayTrend.map((_, i) => {
                                    ...
                                    if (i === total - 1) { label = "THIS WEEK"; opacity = "opacity-60 font-bold"; }
                                    else if (i === total - 2) { label = "LAST WEEK"; opacity = "opacity-40"; }
                        ...
                        <div className="mt-8 flex justify-between items-center border-t border-white/5 pt-4">
                            <span className="text-[9px] text-white/30 font-black uppercase tracking-[0.3em] font-mono whitespace-nowrap">
                                WEEKLY HEAT VOLUME
                            </span>
                            <div className="flex gap-2 items-center">
                                <div className="w-1 h-1 bg-white/60 rounded-full animate-pulse opacity-60" />
                    ...
                    {weeklyGenreViews.length > 0 && (() => {
                        ...
                        return (
                            <div className="w-full border-t border-white/5 pt-6">
                                <p className="text-[9px] font-black text-white/30 uppercase tracking-[0.3em] font-mono mb-4">
                                    GENRE SPLIT
                                </p>
                                    ...
                                    {weeklyGenreViews.map((g, i) => {
                                        ...
                                        return (
                                            <div key={g.genre} className="flex items-center gap-3">
                                                <span className="text-[8px] font-black font-mono text-white/35 w-14 shrink-0 text-right tracking-wider">
                                                ...
                                                <span className="text-[8px] font-mono text-white/25 w-10 shrink-0">
                                                    {fmtViews(g.views)}
                                                </span>
                                            </div>
                                        );

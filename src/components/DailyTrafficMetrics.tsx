...
interface DailyTrafficMetricsProps {
    growth?: number;
    trend?: number[];
    dailyGenreViews?: { genre: string; views: number }[];
}
...
export const DailyTrafficMetrics: React.FC<DailyTrafficMetricsProps> = ({ growth = 0, trend = [], dailyGenreViews = [] }) => {
        ...
        >
            <div className="relative z-10 flex flex-col items-center">
                <div className="flex items-center justify-between mb-6 w-full">
                    <h2 className="text-[10px] font-black tracking-[0.4em] uppercase text-white/80">DAILY TRAFFIC</h2>
                ...
                <div className="flex flex-col items-stretch gap-8 mb-6 w-full">
                    <div className="flex flex-col items-center text-center text-white">
                        ...
                        <p className="text-[8px] md:text-[9px] font-bold text-white/55 uppercase tracking-[0.2em] mt-4">
                            DAILY COMPARISON <span className="text-white/45 ml-1 font-mono">VS YESTERDAY</span>
                        </p>
                    </div>

                    <div className="w-full pt-4 border-t border-white/5">
                        <div className="flex flex-col h-full">
                            <div className="flex items-end justify-between h-24 md:h-32 gap-1 px-1">
                                {displayTrend.map((val, i) => (
                                    <div key={i} className="flex-1 flex flex-col justify-end items-center h-full group/bar relative">
                                        <div className="absolute -top-7 left-1/2 -translate-x-1/2 opacity-0 group-hover/bar:opacity-100 transition-opacity duration-300 pointer-events-none z-20">
                                            <div className="text-[8px] font-mono text-white/95 font-black whitespace-nowrap bg-black/80 px-1 py-0.5 rounded border border-white/10">
                                        ...
                                        <motion.div
                                            ...
                                            transition={{ duration: 1, delay: 0.2 + i * 0.04, ease: [0.33, 1, 0.68, 1] }}
                            ...
                            <div className="flex justify-between px-1 mt-3">
                                {displayTrend.map((_, i) => {
                                    ...
                                    if (i === total - 1) { label = "TODAY"; opacity = "opacity-85 font-bold"; }
                                    else if (i === total - 2) { label = "YEST"; opacity = "opacity-65"; }
                                    else if (i === 0) { label = `${total - 1}D`; }
                                    else if ((total - 1 - i) % 3 === 0) { label = `${total - 1 - i}D`; }
                                    return (
                    ...
                    {/* Genre breakdown */}
                    {dailyGenreViews.length > 0 && (() => {
                        const total = dailyGenreViews.reduce((s, g) => s + g.views, 0) || 1;
                        return (

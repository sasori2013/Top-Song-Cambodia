...
function onOpen() {
    ...
    ui.createMenu('🎬 ランキング管理')
        ...
        .addItem('5. 🧪 AIランキング試作 (実験)', 'generateAiTestRanking')
        .addSeparator()
        .addItem('6. 脚本生成 (buildMcScript)', 'buildMcScript')
        ...
        .addItem('🚀 キャッシュを強制更新 (高速化)', 'updateRankingCache')
...
function setDailyTriggers() {
    ...
    // 毎日 デイリーランキングを生成 (19時45分枠：FB投稿の20時30分枠より確実に先行させる)
    ScriptApp.newTrigger('autoGenerateDailyRanking')
        .timeBased().atHour(19).nearMinute(45).everyDays(1).create();

    // Facebook投稿は Node.js パイプライン (post-fb-node.mjs) が担当するため GAS トリガーは無効
...
function autoPostToFacebook() {
    ...
    try {
        ...
        sendTelegramNotification_(`【実行】Facebook自動投稿を開始します (${todayStr}分 / 高速並列版)`);
        ...
        if (!token) {
            ...
            sendTelegramNotification_('🔴 FB投稿失敗: アクセストークン(FB_ACCESS_TOKEN)がスクリプトプロパティに設定されていません。');
            return;
        }
        
        // ロック取得「後」に最新のプロパティを再チェック (重要: 他のプロセスが待機中に完了した場合を考慮)
        const checkPosted = () => {
          const lastPostDate = props.getProperty('LAST_POST_DATE');
          if (lastPostDate === todayStr) {
              logToSheet_(`FB_POST: スキップ - ${todayStr}分は既に投稿済みです (Idempotency check: Already posted today)`);
              return true;
          }
          const runningFlag = props.getProperty('FB_POST_RUNNING');
          if (runningFlag === 'true') {
              logToSheet_('FB_POST: スキップ - 他のプロセスが現在投稿処理中です (Running flag active)');
              return true;
          }
          return false;
        };

        const checkResult = checkPosted();
        if (checkResult) {
            const reason = checkResult === true ? '既投稿または実行中' : checkResult;
            logToSheet_(`FB_POST: スキップ - 事前チェック不合格 (${reason})`);
            sendTelegramNotification_(`ℹ️ FB投稿スキップ: ${reason}`);
            return;
        }

        // 実行中フラグをセット
        props.setProperty('FB_POST_RUNNING', 'true');
        ...
        sendTelegramNotification_('📸 OG画像生成完了 (4枚)');
        ...
        sendTelegramNotification_(`✅ FB画像アップロード完了 (${photoIds.length}枚)`);
        ...
        // 実行中フラグとロックを解除
        props.deleteProperty('FB_POST_RUNNING');
        lock.releaseLock();
    }
}
...
function doGet(e) {
    try {
        ...
        if (action === 'refresh') {
            refreshAllTriggers();
            return ContentService.createTextOutput(JSON.stringify({ success: true, message: 'All triggers refreshed and FB 20:30 added.' }))
                .setMimeType(ContentService.MimeType.JSON);
        }
        ...
        if (action === 'logs') {
            ...
            let logs = [];
            if (shLog) {
                logs = shLog.getRange(Math.max(1, shLog.getLastRow() - 50), 1, Math.min(50, shLog.getLastRow()), 2).getValues();
            }
            return ContentService.createTextOutput(JSON.stringify({ logs: logs.reverse() }))
                .setMimeType(ContentService.MimeType.JSON);
        }
        ...
        if (action === 'debug_fb') {
            ...
            const debugInfo = {
                LAST_POST_DATE: props['LAST_POST_DATE'] || '-',
                FB_POST_RUNNING: props['FB_POST_RUNNING'] || 'false',
                FB_POST_LOCK: props['FB_POST_LOCK'] || '-',
                HAS_TOKEN: !!props['FB_ACCESS_TOKEN'],
                PROJECT_ID: '1YgFAwxQV2AcXWkksh_Cz24KKlE9rMgcbeYa_iHVOyEJyk-cDwn3C-12Z',
        ...
        if (action === 'update_cache') {
            updateRankingCache();
            return ContentService.createTextOutput(JSON.stringify({ success: true, message: 'Cache updated.' }))
                .setMimeType(ContentService.MimeType.JSON);
        }

        // デフォルト: ランキングデータを返す (キャッシュ優先)
        const ss = SpreadsheetApp.getActiveSpreadsheet();
        const shCache = ss.getSheetByName('SYS_CACHE');
        let dataStr = "";
        
        if (shCache) {
            dataStr = shCache.getRange(1, 1).getValue();
        }
...
function refreshAllTriggers() {
    logToSheet_('SYSTEM: 全トリガーの初期化（リセット）を開始します...');
    setDailyTriggers();
    logToSheet_('SYSTEM: 全トリガーの初期化が完了しました。20:30の古いトリガー等は削除されました。');
    
    try {
        if (SpreadsheetApp.getUi()) {
            SpreadsheetApp.getUi().alert('トリガーの初期化が完了しました。\n古い20:30の投稿トリガー等は削除され、最新の設定が適用されました。');
        }
    } catch (e) {}
}
...
/**
 * ランキングデータを事前計算し、キャッシュシートに保存する (高速化用)
 */
function updateRankingCache() {
    try {
        logToSheet_('SYSTEM: ランキングキャッシュの更新を開始します...');
        const data = getRankingData_();
        const json = JSON.stringify(data);
        
        const ss = SpreadsheetApp.getActiveSpreadsheet();
        let shCache = ss.getSheetByName('SYS_CACHE');
        if (!shCache) {
            shCache = ss.insertSheet('SYS_CACHE');
            shCache.hideSheet(); // ユーザーには見せない
        }
        
        shCache.getRange(1, 1).setValue(json);
        logToSheet_('SYSTEM: ランキングキャッシュの更新が完了しました (Size: ' + json.length + ')');
        
        try {
            if (SpreadsheetApp.getUi()) {
                ss.toast('キャッシュを更新しました', '完了');
            }
        } catch(e) {}
        
    } catch (e) {
        console.error("Failed to update ranking cache: " + e.message);
        logToSheet_('SYSTEM: キャッシュ更新エラー - ' + e.message);
    }
}

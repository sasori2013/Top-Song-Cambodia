...
function autoGenerateDailyRanking() {
  ...
  // すべての生成が終わったらキャッシュを更新する
  if (typeof updateRankingCache === 'function') {
    updateRankingCache();
  }

  // 【リレートリガー】1分後にFacebook投稿を予約
  // これにより「生成完了」を検知して即座に投稿しつつ、6分制限を回避（別実行枠）できます
  try {
    logToSheet_('SYSTEM: ランキング生成完了。Facebook投稿トリガーを1分後に予約します...');
    
    // 既存の同一トリガーがあれば念のため削除（重複防止）
    const triggers = ScriptApp.getProjectTriggers();
    for (let i = 0; i < triggers.length; i++) {
      if (triggers[i].getHandlerFunction() === 'autoPostDailyRankingToFacebook') {
    ...
    logToSheet_('SYSTEM: Facebook投稿トリガー作成エラー - ' + err.message);
  }
}
  ...
  try {
    ...
    if (rankingSheet && rankingSheet.getLastRow() > 1) {
      ...
      if (firstDataDateStr !== todayKey) {
        logToSheet_('autoPostDailyRankingToFacebook: 本日(' + todayKey + ')のランキングがまだ生成されていないため、投稿を待機/スキップします。');
        return;
      }
    } else {
